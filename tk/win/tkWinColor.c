...
static void RefreshColors _ANSI_ARGS_((ClientData clientData));
    ...
    XColor* exact_def_return;
{
    ...
    if (spec[0] == '#') {
	return GetColorByValue(spec, exact_def_return);
    } else {
	return GetColorByName(spec, exact_def_return);
    }
}

/*
    ...
    XColor* color;
{
	...
	if (newPixel != closePixel) {
	    ...
	    if (!cmap->stale) {
		cmap->stale = 1;
		Tk_DoWhenIdle(RefreshColors, (ClientData)cmap);
	    }
	    
	}
	color->pixel = PALETTERGB(entry.peRed, entry.peGreen, entry.peBlue);
	entryPtr = Tcl_CreateHashEntry(&cmap->refCounts,
		(ClientData)color->pixel, &new);
	if (new) {
	    refCount = 1;
	} else {
	...
	color->pixel = GetNearestColor(dc,
		PALETTERGB(entry.peRed, entry.peGreen, entry.peBlue));
	color->red = (GetRValue(color->pixel) << 8);
	color->green = (GetGValue(color->pixel) << 8);
	color->blue = (GetBValue(color->pixel) << 8);
    }
...
XAllocNamedColor(display, colormap, color_name, screen_def_return,
    ...
    XColor* exact_def_return;
{
    int rval = GetColorByName(color_name, exact_def_return);
    if (rval) {
    ...
    Colormap colormap;
{
    TkWinColormap *cmap = (TkWinColormap *) colormap;
    if (cmap->stale) {
	Tk_CancelIdleCall(RefreshColors, (ClientData)cmap);
    }
 ...
 * RefreshColors --
 *
 *	This function is an idle handler which is called to update
 *	a stale colormap.
 *
 * Results:
 *	None.
 *
 * Side effects:
 *	May cause windows to be redrawn.
 *
 *----------------------------------------------------------------------
 */

static void
RefreshColors(clientData)
    ClientData clientData;
{
    TkWinColormap *cmap = (TkWinColormap *) clientData;
    HDC dc;
    HPALETTE oldPalette;
    BOOL background;
	...
	cmap->stale = 0;
	dc = GetDC(NULL);
	if (cmap->palette == TkWinGetSystemPalette()) {
	    background = FALSE;
	} else {
	    background = TRUE;
	}
	oldPalette = SelectPalette(dc, cmap->palette, background);
	RealizePalette(dc);
