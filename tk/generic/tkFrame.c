...
static int		FrameCreate _ANSI_ARGS_((ClientData clientData,
...
{
    return FrameCreate(clientData, interp, argc, argv, 0);
}
...
Tk_ToplevelCmd(clientData, interp, argc, argv)
    ...
    char **argv;		/* Argument strings. */
{
    return FrameCreate(clientData, interp, argc, argv, 1);
}

/*
 *--------------------------------------------------------------
...
static int
FrameCreate(clientData, interp, argc, argv, toplevel)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
