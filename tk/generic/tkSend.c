				 ...
				 * unique. */
{
    ...
    /*
     * The name appears to be in use already.  Check to be sure
     * that it really is in use by someone other than us (perhaps
     * the application died without removing its name from the
     * registry?).
     */

    if ((w == Tk_WindowId(dispPtr->commTkwin))
	    || !ValidateName(winPtr->dispPtr, actualName, w, 1)) {
	RegDeleteName(regPtr, actualName);
	goto gotName;
    }

    /*
     * Start with the suffix " #2" and increment the number until one
     * eventually works.
     */
	...
	if ((w == Tk_WindowId(dispPtr->commTkwin))
		|| !ValidateName(dispPtr, actualName, w, 1)) {
	    RegDeleteName(regPtr, actualName);
	    break;
	}
    }
    ...
    char **argv;			/* Argument strings. */
{
	...
	Tk_Preserve((ClientData) riPtr);
	if (firstArg == (argc-1)) {
	    result = Tcl_GlobalEval(riPtr->interp, argv[firstArg]);
	} else {
	    ...
	    result = Tcl_GlobalEval(riPtr->interp, Tcl_DStringValue(&request));
	    Tcl_DStringFree(&request);
	}
	if ((riPtr->interp != NULL) && (interp != riPtr->interp)) {
	    if (result == TCL_ERROR) {
		/*
		...
		Tcl_SetVar2(interp, "errorCode", (char *) NULL,
			Tcl_GetVar2(riPtr->interp, "errorCode", (char *) NULL,
			TCL_GLOBAL_ONLY), TCL_GLOBAL_ONLY);
	    }
	    if (riPtr->interp->freeProc != TCL_STATIC) {
		interp->result = riPtr->interp->result;
		interp->freeProc = riPtr->interp->freeProc;
		riPtr->interp->freeProc = TCL_STATIC;
	    } else {
		Tcl_SetResult(interp, riPtr->interp->result, TCL_VOLATILE);
	    }
	    Tcl_ResetResult(riPtr->interp);
	}
	Tk_Release((ClientData) riPtr);
	return result;
    }
    ...
    pending.timeout = Tk_CreateTimerHandler(1000, TimeoutProc,
	    (ClientData) &pending);
	...
	Tk_DoOneEvent(TK_X_EVENTS|TK_TIMER_EVENTS);
    }
    Tk_DeleteTimerHandler(pending.timeout);
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if ((*p == 'c') && (p[1] == 0)) {
	    ...
	    Tk_Preserve((ClientData) riPtr);
	    result = Tcl_GlobalEval(riPtr->interp, script);
	    if ((commWindow != None) && (riPtr->interp != NULL)) {
		Tcl_DStringAppend(&reply, riPtr->interp->result, -1);
		if (result == TCL_ERROR) {
		    char *varValue;
    
		    varValue = Tcl_GetVar2(riPtr->interp, "errorInfo",
			    (char *) NULL, TCL_GLOBAL_ONLY);
		    if (varValue != NULL) {
			Tcl_DStringAppend(&reply, "\0-i ", 4);
			Tcl_DStringAppend(&reply, varValue, -1);
		    }
		    varValue = Tcl_GetVar2(riPtr->interp, "errorCode",
			    (char *) NULL, TCL_GLOBAL_ONLY);
		    if (varValue != NULL) {
				 ...
				 * responded to. */
{
	...
	if (!ValidateName(pcPtr2->dispPtr, pcPtr2->target,
	    ...
	    if (ValidateName(pcPtr2->dispPtr, pcPtr2->target,
		    pcPtr2->commWindow, 1)) {
		msg = "can't send to old Tk applications (versions before 4.0)";
	    } else {
	    ...
	    Tk_CreateTimerHandler(2000, TimeoutProc, clientData);
	}
    }
}

				 ...
				 * as ClientData. */
{
    ...
    Tk_EventuallyFree((ClientData) riPtr, TCL_DYNAMIC);
}

/*
 *----------------------------------------------------------------------
