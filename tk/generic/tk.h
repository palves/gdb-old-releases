...
/*
 * Time structure used by Tk_NotifyTimer and the Tk_TimerEvent structure.
 * Represents the number of seconds since Jan 1, 1970 GMT.
 */
...
/*
 * Event structures for different Tk event types.  In each of these
 * structures, type is one of TK_WINDOW_EVENTS, TK_FILE_EVENTS, or
 * TK_TIMER_EVENTS.
 */

typedef struct Tk_TimerEvent {
    int type;			/* TK_TIMER_EVENTS */
    Tk_Time time;		/* Time that event was generated. */
} Tk_TimerEvent;

typedef struct Tk_FileEvent {
    int type;			/* TK_FILE_EVENTS */
    int fd;			/* I/O channel for which event occured. */
    int mask;			/* Combination of TK_READABLE, TK_WRITABLE,
				 * TK_EXCEPTION.*/
} Tk_FileEvent;

typedef struct Tk_WindowEvent {
    int type;			/* TK_WINDOW_EVENTS */
    XEvent event;		/* Window system event. */
} Tk_WindowEvent;

typedef union Tk_Event {
    int type;
    Tk_FileEvent file;
    Tk_TimerEvent timer;
...
typedef enum {
    TK_QUEUE_TAIL, TK_QUEUE_HEAD, TK_QUEUE_MARK
} Tk_QueuePosition;
...
EXTERN int		Tk_IdlePending _ANSI_ARGS_((void));
...
EXTERN void		Tk_PreserveColormap _ANSI_ARGS_((Display *display,
			    Colormap colormap));
EXTERN void		Tk_QueueEvent _ANSI_ARGS_((Tk_Event *event,
...
EXTERN void		Tk_NotifyDisplay _ANSI_ARGS_((Display *display));
EXTERN void		Tk_NotifyFile _ANSI_ARGS_((int fd, int mask));
...
EXTERN int		Tk_ServiceEvent _ANSI_ARGS_((int mask));
EXTERN int		Tk_ServiceIdle _ANSI_ARGS_((void));
...
EXTERN void		Tk_SetInternalBorder _ANSI_ARGS_((Tk_Window tkwin,
			    int width));
EXTERN void		Tk_NotifyTimer _ANSI_ARGS_((Tk_Time *time));
...
EXTERN void		Tk_IgnoreDisplay _ANSI_ARGS_((Display *display));
EXTERN void		Tk_IgnoreFile _ANSI_ARGS_((int fd));
