    ...
    TkCanvas *canvasPtr;		/* Information about canvas. */
{
	...
	PrintScrollFractions(canvasPtr->xOrigin + canvasPtr->inset,
		canvasPtr->xOrigin + Tk_Width(canvasPtr->tkwin)
		- canvasPtr->inset, canvasPtr->scrollX1, canvasPtr->scrollX2,
		buffer);
	result = Tcl_VarEval(canvasPtr->interp, canvasPtr->xScrollCmd, " ",
		buffer, (char *) NULL);
	if (result != TCL_OK) {
	...
	PrintScrollFractions(canvasPtr->yOrigin + canvasPtr->inset,
		canvasPtr->yOrigin + Tk_Height(canvasPtr->tkwin)
		- canvasPtr->inset, canvasPtr->scrollY1, canvasPtr->scrollY2,
		buffer);
	result = Tcl_VarEval(canvasPtr->interp, canvasPtr->yScrollCmd, " ",
		buffer, (char *) NULL);
	if (result != TCL_OK) {
	    Tk_BackgroundError(canvasPtr->interp);
	}
