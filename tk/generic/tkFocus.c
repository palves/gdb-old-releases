...

{
    ...
    e.window.event.xfocus.serial = LastKnownRequestProcessed(winPtr->display);
    e.window.event.xfocus.send_event = GENERATED_EVENT_MAGIC;
    e.window.event.xfocus.display = winPtr->display;
    e.window.event.xfocus.mode = NotifyNormal;
    TkInOutEvents(&e, sourcePtr, destPtr, FocusOut, FocusIn, TK_QUEUE_MARK);
}

/*
 *----------------------------------------------------------------------
