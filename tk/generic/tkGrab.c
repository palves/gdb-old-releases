					 ...
					 * where event was reported. */
{
	...
	if (winPtr2 != winPtr) {
	    ...
	    newEvent.type = TK_WINDOW_EVENTS;
	    newEvent.window.event = *eventPtr;
	    TkChangeEventWindow(&newEvent.window.event, winPtr2);
	    Tk_QueueEvent(&newEvent, TK_QUEUE_HEAD);
	    return 0;
	}
	return 1;
    }
	...
	if (eventPtr->type == ButtonPress) {
		...
		if (outsideGrabTree) {
		    ...
		    newEvent.type = TK_WINDOW_EVENTS;
		    newEvent.window.event = *eventPtr;
		    TkChangeEventWindow(&newEvent.window.event,
			    dispPtr->grabWinPtr);
		    Tk_QueueEvent(&newEvent, TK_QUEUE_HEAD);
		    return 0;					/* Note 2. */
		}
...
TkInOutEvents(eventPtr, sourcePtr, destPtr, leaveType, enterType, position)
    Tk_Event *eventPtr;		/* A template X event.  Must have all fields
    ...
    Tk_QueuePosition position;	/* Position at which events are added to
...
#define QUEUE(w, t, d)						\
    if (w->window != None) {					\
	eventPtr->window.event.type = t;			\
	if (focus) {						\
	    eventPtr->window.event.xfocus.window = w->window;	\
	    eventPtr->window.event.xfocus.detail = d;		\
	} else {						\
	    eventPtr->window.event.xcrossing.detail = d;	\
	    TkChangeEventWindow(&eventPtr->window.event, w);	\
	}							\
	Tk_QueueEvent(eventPtr, position);			\
    }
 ...
 *
 * Side effects:
 *	Updates the grabWinPtr for the display of the given event.
 *
 *----------------------------------------------------------------------
 */
    ...
    XEvent *eventPtr;		/* Pointer to the trigger event. */
{
    ...
    /*
     * Check if this is a special event to change grabWinPtr.
     */

    if (eventPtr->xany.send_event == GRAB_WINDOW_EVENT_MAGIC) {
	dispPtr = TkGetDisplay(eventPtr->xany.display);
	dispPtr->grabWinPtr = (TkWindow *) Tk_IdToWindow(
	    eventPtr->xany.display, eventPtr->xany.window);
    }
}

/*
				 ...
				 * generate enter events. */
{
    ...
    e.window.event.xcrossing.serial = LastKnownRequestProcessed(
	winPtr->display);
    e.window.event.xcrossing.send_event = GENERATED_EVENT_MAGIC;
    e.window.event.xcrossing.display = winPtr->display;
    e.window.event.xcrossing.root = RootWindow(winPtr->display,
	    winPtr->screenNum);
    e.window.event.xcrossing.time = TkCurrentTime(winPtr->dispPtr);
    XQueryPointer(winPtr->display, winPtr->window, &dummy1, &dummy2,
	    &e.window.event.xcrossing.x_root, &e.window.event.xcrossing.y_root,
	    &dummy3, &dummy4, &e.window.event.xcrossing.state);
    e.window.event.xcrossing.mode = mode;
    e.window.event.xcrossing.focus = False;
    TkInOutEvents(&e, sourcePtr, destPtr, (leaveEvents) ? LeaveNotify : 0,
	    (enterEvents) ? EnterNotify : 0, TK_QUEUE_MARK);
}

/*
 *----------------------------------------------------------------------
				 ...
				 * number at least this great. */
{
    ...
    while (Tk_DoOneEvent(TK_DONT_WAIT|TK_WINDOW_EVENTS)) {
    }
 ...
 *	for a display that have mode NotifyGrab or NotifyUngrab and
    ...
    XEvent *eventPtr;
{
    ...
    /*
     * We always want to keep events generated by the grab module.
     */

    if ((eventPtr->xany.send_event == GRAB_WINDOW_EVENT_MAGIC)
	    || (eventPtr->xany.send_event == GENERATED_EVENT_MAGIC)) {
				 ...
				 * window (may be NULL). */
{
    ...
    event.window.event.xany.type = -1;
    event.window.event.xany.display = dispPtr->display;
    event.window.event.xany.send_event = GRAB_WINDOW_EVENT_MAGIC;
    event.window.event.xany.window
	= (grabWinPtr == NULL) ? None : grabWinPtr->window;
    Tk_QueueEvent(&event, TK_QUEUE_MARK);
 ...
 *	subtree of the window heirarchy.
 *
 * Results:
 *
