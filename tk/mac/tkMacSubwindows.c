...
static void MacMoveWindow _ANSI_ARGS_((WindowPtr window, int x, int y));
 ...
 * TkMacGetXWindow --
 *
 *	Returns the X window Id associated with the given WindowPtr.
 *
 * Results:
    ...
    Window parent;
{
	...
	TkGenWMMoveRequestEvent((Tk_Window) macWin->winPtr,
		(int) geometry.left, (int) geometry.top);
...
XDestroyWindow(Display *display, Window window)
{
	...
	DisposeWindow((WindowPtr) destPort);
	macWin->portPtr = NULL;
	...
	macWin->winPtr->parentPtr->privatePtr->clipValid = false;
    ...
    unsigned int height;
{
	 ...
	 */
	SizeWindow((WindowPtr) destPort, (short) width, (short) height, false);
...
{	
	 ...
	 */
	SizeWindow((WindowPtr) destPort, (short) width, (short) height, false);
	MacMoveWindow((WindowPtr) destPort, x, y);
 ...
 *  TkMacUpdateClipRgn --
 *
 *	This function updates the cliping regions for a given window and
 *  all of its children.  Once updated the clipValid flag in the
 *  subwindow data structure is set to true.  The clipValid flag
 *  should always be true before any drawing is attempted.
 *
 * Results:
 *	None.
 *
 * Side effects:
 *	The clip region is updated.
 *
 *----------------------------------------------------------------------
 */
    ...
    TkWindow *winPtr;
{
    ...
    if (winPtr->privatePtr->clipValid == false) {
	...
	DisposeRgn(tempRgn);
	winPtr->privatePtr->clipValid = true;
    }
}

/*
 ...
 * InvalClipRgns --
 *
 *	This function invalidates the clipping regions for a given window
 *  and all of its children.  This function should be called whenever
 *  changes are made to subwindows that would effect the size or 
 *  position of windows.
 *
 * Results:
 *	None.
    ...
    TkWindow *winPtr;
{
    ...
    winPtr->privatePtr->clipValid = false;
...
void 
MacMoveWindow(WindowPtr window, int x, int y)
{
    int offset;
    ...
    int deltaX, deltaY;
{
    ...
    nextPtr = winPtr->childList;
    while (nextPtr != NULL) {
	UpdateOffsets(nextPtr, deltaX, deltaY);
