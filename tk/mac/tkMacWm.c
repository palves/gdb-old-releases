...
void TkSetWMName _ANSI_ARGS_((TkWindow *winPtr, Tk_Uid titleUid));
    ...
    char **argv;		/* Argument strings. */
{
	...
	cmapList = (Window *) ckalloc((unsigned) (windowArgc*sizeof(Window)));
				 ...
				 * unmapped. */
{
	...
	wmPtr->flags |= WM_SYNC_PENDING;
	Tk_ServiceEvent(TK_WINDOW_EVENTS);
	wmPtr->flags &= ~WM_SYNC_PENDING;
    }
...
{
    /* 
     * The Mac port will look directly in the
     * wmPtr for this information - no update is
     * necessary.
     */
}

...
{
    /*
     * The Maintosh port will just look for properties in the wmInfoPtr
     * directly.  Therefor no update is necessary.
     */
}

/*
				 ...
				 * Should not be a top-level window. */
{
    ...
    for (topPtr = winPtr->parentPtr; !(topPtr->flags & TK_TOP_LEVEL);
    ...
    Rect *geometry;
{
    ...
    if (winPtr->wmInfoPtr->sizeHintsFlags & USPosition) {
	x = winPtr->changes.x;
	y = winPtr->changes.y;
    } else {
	/* 
    ...
    Point start;
{
	...
	bounds.left = (winPtr->wmInfoPtr->minWidth < 64) ? 64 :
	    winPtr->wmInfoPtr->minWidth;
	bounds.top = (winPtr->wmInfoPtr->minHeight < 64) ? 64 :
	    winPtr->wmInfoPtr->minHeight;
	bounds.right = (winPtr->wmInfoPtr->maxWidth < 64) ? 64 :
	    winPtr->wmInfoPtr->maxWidth;
	bounds.bottom = (winPtr->wmInfoPtr->maxHeight < 64) ? 64 :
	    winPtr->wmInfoPtr->maxHeight;
	...
	if (growResult != 0) {
	    ...
	    TkGenWMResizeRequestEvent(tkwin, 
...
void
TkGenWMResizeRequestEvent(tkwin, width, height)
    Tk_Window tkwin;
    int width;
    int height;
{
    ...
    event->xconfigure.border_width = 0;		/* Tk assumes border_width is always 0 */
    
    Tk_QueueEvent(&tkEvent, TK_QUEUE_TAIL);
}

void
TkGenWMMoveRequestEvent(tkwin, x, y)
    Tk_Window tkwin;
    int x;
    int y;
{
