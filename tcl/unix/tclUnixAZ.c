 ...
 * in Tk to destroy file event bindings whenever a file is closed.  It
 * is set by the Tcl_SetFileCloseProc procedure.  I realize that this
 * is a big ugly...
 */

void (*fileCloseProc) _ANSI_ARGS_((FILE *f)) = NULL;
...
static char *		ComputeFileName _ANSI_ARGS_((int fd));

/*
 *----------------------------------------------------------------------
 *
 * ComputeFileName --
 *
 *	Given a file number computes the hash key for the entry for
 *	this file in the file table.
 *
 * Results:
 *	A string containing the hash key
 *
 * Side effects:
 *	None.
 *
    ...
    char **argv;			/* Argument strings. */
{
    ...
    if (hTablePtr == (Tcl_HashTable *) NULL) {
        panic("Internal error: Could not find file table\n");
    }
    
    hPtr = Tcl_FindHashEntry(hTablePtr, ComputeFileName(fileno(f)));
     ...
     * calls to *fileCloseProc are a hack so that Tk's "fileevent"
     * command can clean up its state, if any, when files are closed.
     */

    if (oFilePtr->f2 != NULL) {
	if (fileCloseProc != NULL) {
	    (*fileCloseProc)(oFilePtr->f2);
	}
...
#ifdef MAC_TCL
	Tcl_AppendResult(interp, "cannot support pipes under Mac OS",
		(char *)NULL);
#else /* !MAC_TCL */
 ...
 *----------------------------------------------------------------------
 *
 * Tcl_SetFileCloseProc --
 *
 *	This routine is a secret trap-door used by the "fileevent"
 *	command in Tk to destroy file event bindings whenever a file
 *	is closed.
 *
 * Results:
 *	None.
 *
 * Side effects:
 *	Sets fileCloseProc so Tk will be informed when files are
 *	being closed.
 *
 *----------------------------------------------------------------------
 */
...
void
Tcl_SetFileCloseProc(proc)
    void (*proc) _ANSI_ARGS_((FILE *f));
{
    fileCloseProc = proc;
}
