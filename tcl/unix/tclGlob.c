...
/* CYGNUS LOCAL dje/win32 */
	    ...
	    /*
	     * This is a special hack for DOS machines.  A leading
	     * letter (here we use any char -- is this a bug?)
	     * followed by a colon is treated like an absolute path.
	     */
            char drive[4];
	    drive[0] = thisName[0];
	    drive[1] = ':';
	    drive[2] = thisName[2] == '/' ? '/' : 0;
	    drive[3] = 0;
	    result = DoGlob(interp, drive,
			 thisName + 2 + (thisName[2] == '/'));
#endif
/* END CYGNUS LOCAL */
	} else {
