				 ...
				 * myName. */
{
    ...
    if (iPtr->varFramePtr == ((flags & TCL_GLOBAL_ONLY) ? NULL : framePtr)) {
	varPtr->flags |= VAR_SAME_TABLE;
    }
				 ...
				 * delete. */
{
	 ...
	 * the referenced variable if it's no longer needed.  Don't touch
	 * the other variable if it's in the same table as us:  it will
	 * be deleted anyway, and in fact it might have been deleted
	 * already.
	 */

	if ((varPtr->flags & VAR_UPVAR) && !(varPtr->flags & VAR_SAME_TABLE)) {
	    upvarPtr = varPtr->value.upvarPtr;
	    upvarPtr->refCount--;
	    if (upvarPtr->flags & VAR_UNDEFINED) {
		CleanupVar(upvarPtr, (Var *) NULL);
	...
	/*
	 * Note: recycle the memory space even if the reference count
	 * is non-zero.  The only references that should be outstanding
	 * at this point are upvars from other variables in the same
	 * table, and we'll skip them in the code above.
	 */
