 ...
 * VAR_SAME_TABLE -		1 means that this is an upvar that references
 *				another variable in the same table.  This
 *				information is needed when the table is
 *				deleted so that we don't cleanup the entry
...
typedef struct TclOpenFile {
    ...
    int refCount;		/* How many interpreters have this file
                                 * open? */
} TclOpenFile;
...
extern void		panic();
extern int              TclCheckForPipes _ANSI_ARGS_((void));
extern void		TclCopyAndCollapse _ANSI_ARGS_((int count, char *src,
			    char *dst));
extern int		TclCreateTempFile _ANSI_ARGS_((void));
extern int              TclCreatePipe _ANSI_ARGS_((int *pipeIds));
...
extern int              TclSpawnPipeline _ANSI_ARGS_((Tcl_Interp *interp,
	                    ...
	                    int *errorId, int *inputId, int *outputId,
	                    int *closeInput, int *pipeIds, int *errpipeIds,
			    int *maxFd, char *IntIn, char *FinalOut));
