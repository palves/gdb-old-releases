 ...
 * Tcl_Volatile --
 *
 *	This is a dummy procedure, in that it should never be
 *	invoked.  It is simply a placeholder.  It provides an
 *	address for TCL_VOLATILE to refer to, which won't be
 ...
 *	Otherwise the return value is 1 if 
 *
 * Side effects:
 *	None.
 *
					 ...
					 * the result of interp. */
{
	...
	if (iPtr->freeProc == (Tcl_FreeProc *) free) {
	    dsPtr->string = iPtr->result;
	    dsPtr->spaceAvl = dsPtr->length+1;
	} else {
	    dsPtr->string = ckalloc((unsigned) (dsPtr->length+1));
