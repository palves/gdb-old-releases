 ...
 * getenv --
 *
 *  Gets the value of a named environment variable.  POSIX 4.6.1
 *
 * Results:
    ...
    const char *name;
{
	...
	len = (int) ((char *) strchr(environ[i], '=') - environ[i]);
    ...
    long whichFolder;
{
	...
	HLock(theString);
	result = (char *) ckalloc(strlen(prefixTag) + size + 1);
	strcpy(result, prefixTag);
	strncpy(result + strlen(prefixTag), *theString, size);
	result[strlen(prefixTag) + size] = '\0';
	HUnlock(theString);
