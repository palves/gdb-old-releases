...
{
    /*
     * Create macintosh commands not yet in Tcl core.
     */
	Tcl_CreateCommand(interp, "ls", Tcl_LsCmd, NULL, NULL);
	Tcl_CreateCommand(interp, "mkdir", Tcl_MkdirCmd, NULL, NULL);
	Tcl_CreateCommand(interp, "rmdir", Tcl_RmdirCmd, NULL, NULL);
	Tcl_CreateCommand(interp, "rm", Tcl_RmCmd, NULL, NULL);
	Tcl_CreateCommand(interp, "cp", Tcl_CpCmd, NULL, NULL);
	Tcl_CreateCommand(interp, "mv", Tcl_MvCmd, NULL, NULL);
	Tcl_CreateCommand(interp, "echo", Tcl_EchoCmd, NULL, NULL);
    ...
    if (Tcl_Eval(interp, "source -rsrc {Init}") == TCL_ERROR) {
	return TCL_ERROR;
    }
    ...
    if (Tcl_Eval(interp, "source -rsrc {tclshrc}") == TCL_ERROR) {
	return TCL_ERROR;
    }
    return TCL_OK;
}
