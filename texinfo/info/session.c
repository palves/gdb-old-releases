     ...
     WINDOW *window;
{
  ...
  if (info_win)
	...
	info_windows[i] = info_windows[++i];
      ...
      if (info_win->nodes)
	{
#if defined (NODES_ARE_ALWAYS_COPIED)
	  for (i = 0; info_win->nodes[i]; i++)
	    free (info_win->nodes[i]);
#endif /* NODES_ARE_ALWAYS_COPIED */
	  free (info_win->nodes);
...
info_delete_window_internal (window)
     WINDOW *window;
{
  if (!windows->next || (window->flags & W_WindowIsPerm))
    return;
...
DECLARE_INFO_COMMAND (info_last_node, "Select the last node in this file")
{
  ...
  if (fb && fb->nodes)
    {
      for (i = 0; fb->nodes[i]; i++);
      node = info_get_node (fb->filename, fb->nodes[i - 1]->nodename);
    }
  else if (fb && fb->tags)
    {
      for (i = 0; fb->tags[i]; i++);
...
DECLARE_INFO_COMMAND (info_first_node, "Select the first node in this file")
{
  ...
  if (fb && fb->nodes)
    node = info_get_node (fb->filename, fb->nodes[0]->nodename);
  else if (fb && fb->tags)
     ...
     int dir;
{
  ...
  if (file_buffer->tags)
      ...
      while (1)
	{
	  ...
	  if (!echo_area_is_active &&
	      (strcmp (last_subfile, tag->filename) != 0))
	    {
	      window_message_in_echo_area
		("Searching subfile \"%s\"...",
  ...
  /* If this file buffer has a nodes list, search that. */
  if (file_buffer->nodes)
    {
      register int current_node, number_of_nodes;
      NODE_ENTRY *entry;

      /* Find number of nodes, and current node. */
      entry = (NODE_ENTRY *)NULL;
      for (i = 0; file_buffer->nodes[i]; i++)
	if (strcmp (initial_nodename, file_buffer->nodes[i]->nodename) == 0)
	  {
	    current_node = i;
	    entry = file_buffer->nodes[i];
	  }
      number_of_nodes = i;
	...
	return (-1);
      ...
      /* Search through the subsequent nodes, wrapping around at the last
	 node until we find the string or return to this window's node. */
      while (1)
	{
	  NODE *node;

	  /* Allow C-g to exit the search, returning a failure. */
	  return_if_control_g (-1);
	  current_node += dir;

	  if (current_node < 0)
	    current_node = number_of_nodes - 1;
	  else if (current_node == number_of_nodes)
	    current_node = 0;

	  entry = file_buffer->nodes[current_node];
	  ...
	  if (!node)
	      ...
	      if (!echo_area_is_active)
		{
		  if (info_recent_file_error)
		    ...
		    info_error (CANT_FIND_NODE, entry->nodename);
		}
	      return (-1);
	    }
	  ...
	  /* No.  Free NODE and return failure if that is right. */
	  free (node);

	  /* Has the search wrapped around to where it started? */
	  if (strcmp (initial_nodename, entry->nodename) == 0)
	    return (-1);
	}
    }
  return (-1);
