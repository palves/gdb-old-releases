...
static int echo_area_stack_depth ();
     ...
     WINDOW *window;
{
  /* Only do this if the calling window is not the completions window, or,
     if it is the completions window, there is no other window. */
...
restore_calling_window ()
{
  ...
  /* Delete the completions window if it is still present, there aren't
     any prior echo area reads pending, and it isn't the last window on
     the screen. */
  if (compwin)
    {
      if ((compwin != windows || windows->next) &&
	  (echo_area_stack_depth () == 0))
	{
	  WINDOW *next;
...
DECLARE_INFO_COMMAND (ea_possible_completions, "List possible completions")
{
  ...
  if (!completions_found_index)
	...
	if (possible_completions_output_node)
	  {
	    if (possible_completions_output_node != calling_window_node)
	      free (possible_completions_output_node);
	  }
