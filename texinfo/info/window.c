     ...
     NODE *node;
{
  ...
  window->modeline = (char *)xmalloc (window->width);
     ...
     WINDOW *window;
{
  ...
  /* Calculate the maximum size of the information to stick in MODELINE. */
  {
    ...
    if (!parent && !*filename)
      sprintf (modeline, "-----Info: %s, %d lines --%s--",
	       nodename, window->line_count, location_indicator);
    else
      sprintf (modeline, "--%s-Info: (%s)%s, %d lines --%s--",
    ...
    if (i >= window->width)
      modeline[window->width - 1] = '\0';
    else
      {
	while (i < (window->width - 1))
	  modeline[i++] = '-';
