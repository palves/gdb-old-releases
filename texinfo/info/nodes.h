...
/* For each file that we have loaded, we keep a list of the names of the
   nodes that are found in this file.  This is akin to a local (i.e., a
   non-indirect) tags table.  The following structure describes a single
   node within a file. */
typedef struct {
  char *nodename;		/* The nodename within this file. */
  long nodestart;		/* Offset to the start of this node. */
  long nodelen;			/* The length of this node in characters. */
} NODE_ENTRY;

/* Many Info files have an indirect tags table.  The tags table contains
   pointers to different files which actually contain the nodes.  This is
   a structure describing a complete tag.  Indirect tag tables are decomposed
   into arrays of these. */
typedef struct {
  char *nodename;		/* Node pointed to by this tag. */
  char *filename;		/* Filename where this node can be found. */
...
typedef struct {
  ...
  NODE_ENTRY **nodes;		/* List of nodes found in this file. */
  int tags_slots;		/* Number of slots allocated for TAGS. */
  int nodes_slots;		/* Number of slots allocated for NODES. */
