     ...
     WINDOW *window;
{
  ...
  fn_node = make_footnotes_node (window->node);

  /* If we are in the footnotes window, just pretend everything
     is hunky-dory. */
  if (fn_win == window)
    return (FN_FOUND);

  /* If there is no footnotes for this window, delete the old footnotes
     which are appearing on the screen. */
  if (!fn_node && fn_win)
    {
      if (windows->next)
	{
	  free (fn_win->node->contents);
	  free (fn_win->node);
  ...
  if (fn_node && !fn_win)
    {
	 ...
	 contain the footnotes. */
      ...
      active_window = last;
      fn_win = window_make_window (fn_node);
      active_window = old_active;

      if (!fn_win)
	{
	  free (fn_node->contents);
	  free (fn_node);
  ...
  if (fn_node && fn_win)
    {
      window_set_node_of_window (fn_win, fn_node);

      window_change_window_height
	(fn_win, fn_win->line_count - fn_win->height);

      remember_window_and_node (fn_win, fn_node);
    }
