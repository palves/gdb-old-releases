     ...
     struct sym_fns *sf;
{
  ...
  info = (struct coff_symfile_info *)sf->sym_private;
    ...
    long offset;
{
  ...
  if (val != sizeof buffer || buffer < sizeof buffer)
    return 0;
  ...
  if (buffer == sizeof buffer)		/* Empty table -- just the count */
    return 0;
