...
%$Id: refcard.tex,v 1.99 1993/07/01 21:59:02 pesch Exp $
...
\newdimen\fullhsize
\newdimen\secskip \newdimen\itmwid \newdimen\dfnwid 
\newdimen\lskip                                     % used in \def\sec
%-------- Papersize defs:
% Dimensions for landscape printing on 8.5x11in ("US letter") paper

% We want output .25 inch *from paper edge*; i.e. -.75in from TeX default
\hoffset=-0.75in \voffset=-0.75in
\fullhsize=10.5in \hsize=3.3in
\vsize=8in

% Units for \sec definition
\secskip=1pc % space between refcard secs
\itmwid=1.1in  % width of halign col 1
\dfnwid=2.1in  % width of halign col 2
...
%
% dimensions secskip, itmwid, dfnwid, lskip from refpaper input file
...
%COL 2 (of halign):
&\vtop{\hsize=\dfnwid\hangafter=1\hangindent=0.5em
